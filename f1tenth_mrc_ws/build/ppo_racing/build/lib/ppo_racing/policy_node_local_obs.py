#!/home/f1tenth/torch_env/bin/python
import rclpy
...
class PolicyNode(Node):
    def __init__(self):
        ...
        # Load model
        self.actor = ActorNetwork(input_dims=162, n_actions=2, alpha=3e-4, chkpt_dir = '/home/f1tenth/f1tenth_mrc_ws/src/ppo_racing/ppo_racing/tmp/ppo23_dobar')
        ...
        self.pose_sub = self.create_subscription(
            Odometry, '/opti_odom', self.pose_callback, 10)
        
        self.opp_sub = self.create_subscription(Odometry, '/opp_odom', self.opp_pose_callback, 10)

        self.control_pub = self.create_publisher(
            AckermannDriveStamped, '/disparity_vel', 10)
        ...
        self.vel = None
        self.opp_pose = None
        self.opp_pose = np.array([0.0,0.0,0.0])
        self.opp_vel = None
        self.opp_vel = np.array([0.0,0.0,0.0])
        ...
        '''
        waypoints = np.loadtxt('/home/f1tenth/f1tenth_mrc_ws/raceline.csv', delimiter=',')
        self.WAYPOINTS = waypoints[:, :2].flatten() / 100.0
    ...
    def lidar_callback(self, msg):
        ...
        self.scan_short = np.clip(np.array(self.scan_short) / 30.0, 0.1, 1.0)
        self.get_logger().info("got_scan")
    ...
    def pose_callback(self, msg):
        ...
        self.get_logger().info("got_ego_pose")
        ...
        #rel_pose = np.array([-0.03, 0.0, 0.0])
        ...
        #opp_vel = np.array([0.0, 0.0, 0.0])

        vel = np.array([self.vel[0]/self.max_speed, self.vel[1]/self.max_speed, self.vel[2]/np.pi])
    ...
    def opp_pose_callback(self, msg):
        ...
        self.get_logger().info("got_opp_pose")
