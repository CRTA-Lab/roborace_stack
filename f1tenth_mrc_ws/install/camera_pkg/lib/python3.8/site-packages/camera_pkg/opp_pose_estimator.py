...
class AprilTagPosePublisher(Node):
    def __init__(self):
        ...
        self.extraction_radius = 0.8
        self.search_window = [(-0.4, 0.4), (-0.4, 0.4), (-0.4, 0.4)]
        self.feature_points = []
        self.camera_vision = False
    ...
    def timer_callback(self):
        ...
        if not tags:
            if self.camera_vision and self.feature_points==[]:
                self.get_logger().info(f"started extraction")
                self.feature_points = self.extract_opponent_points()
                self.camera_vision = False
                self.get_logger().info(f"Finished extraction: {self.feature_points}")
            lidar_pose = []
            if not self.feature_points == []:
                self.get_logger().info(f"started scan matching")
                lidar_pose = self.match_scan_safe()
                self.get_logger().info(f"Finished scan matching")
            if not lidar_pose:
                odom_msg = Odometry()
            ...
            transformed_odom = self.transform_opp_odometry(odom_msg, self.tf_buffer)
            if transformed_odom:
                self.publisher_.publish(transformed_odom)

                self.get_logger().info(f"Published pose from lidar x:{transformed_odom.pose.pose.position.x}, y:{transformed_odom.pose.pose.position.y}, z:{transformed_odom.pose.pose.position.z}")
            ...
            for tag in tags:
                ...
                odom_msg.child_frame_id = f"opp_pose"
                ...
                yaw_offset = np.pi/2  # 90 degrees
                q_rot = tf_transformations.quaternion_from_euler(0, -yaw_offset, 0)

                # Multiply quaternions (q_rot * q_orig)
                q = tf_transformations.quaternion_multiply(q_rot, q)
                ...
                if transformed_odom:
                    ...
                    self.get_logger().info(f"Published pose x:{transformed_odom.pose.pose.position.x}, y:{transformed_odom.pose.pose.position.y}, z:{transformed_odom.pose.pose.position.z}")
                self.feature_points = []
    ...
    def match_scan_safe(self, resolution=0.05, max_pose_delta=1, max_yaw_delta=1):
        """
        ...
        if delta_dist > max_pose_delta or delta_yaw > max_yaw_delta or best_score > 30.0:
            # Pose moved too much, likely lost track
            return []

        return best_pose

    def transform_opp_odometry(self, odom_msg, tf_buffer, target_frame='base_link'):
        ...
        try:
            ...
            new_odom.pose.pose = transformed_pose
